...
int main(int argc, const char* argv[]) {
  ...
  BenchResult def =
      run_one(kDefaultTg, kDefaultAnchorK, kDefaultVariants, "default");
  printf("== default ==\n");
  printf("  %-16s %12.0f %16.0f %10.4f\n\n", "default", def.j_per_sec,
         def.ec_adds_per_sec, def.wall_seconds);

  // JSON one-liner for downstream regression gates (A47).
  printf("{\"bench\":\"sweep_bench\",\"tg\":%u,\"anchor_k\":%u,"
         "\"variants\":%u,\"range\":%llu,\"j_per_sec\":%.0f,"
         "\"ec_adds_per_sec\":%.0f,\"wall_s\":%.6f}\n",
         kDefaultTg, kDefaultAnchorK, kDefaultVariants,
         (unsigned long long)kAutotuneRange, def.j_per_sec, def.ec_adds_per_sec,
         def.wall_seconds);
